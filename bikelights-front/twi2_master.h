...
#ifndef TWI2_MASTER_H
#define TWI2_MASTER_H
...
#define TWI2_READ_BIT                 (0x01)        //!< If this bit is set in the address field, transfer direction is from slave to master.

#define TWI2_ISSUE_STOP               ((bool)true)  //!< Parameter for @ref twi_master_transfer
#define TWI2_DONT_ISSUE_STOP          ((bool)false) //!< Parameter for @ref twi_master_transfer
...
/*lint ++flb "Enter library region" */
#define TWI2_SCL_HIGH()   do { NRF_GPIO->OUTSET = (1UL << TWI2_MASTER_CONFIG_CLOCK_PIN_NUMBER); } while(0)   /*!< Pulls SCL line high */
#define TWI2_SCL_LOW()    do { NRF_GPIO->OUTCLR = (1UL << TWI2_MASTER_CONFIG_CLOCK_PIN_NUMBER); } while(0)   /*!< Pulls SCL line low  */
#define TWI2_SDA_HIGH()   do { NRF_GPIO->OUTSET = (1UL << TWI2_MASTER_CONFIG_DATA_PIN_NUMBER);  } while(0)   /*!< Pulls SDA line high */
#define TWI2_SDA_LOW()    do { NRF_GPIO->OUTCLR = (1UL << TWI2_MASTER_CONFIG_DATA_PIN_NUMBER);  } while(0)   /*!< Pulls SDA line low  */
#define TWI2_SDA_INPUT()  do { NRF_GPIO->DIRCLR = (1UL << TWI2_MASTER_CONFIG_DATA_PIN_NUMBER);  } while(0)   /*!< Configures SDA pin as input  */
#define TWI2_SDA_OUTPUT() do { NRF_GPIO->DIRSET = (1UL << TWI2_MASTER_CONFIG_DATA_PIN_NUMBER);  } while(0)   /*!< Configures SDA pin as output */
#define TWI2_SCL_OUTPUT() do { NRF_GPIO->DIRSET = (1UL << TWI2_MASTER_CONFIG_CLOCK_PIN_NUMBER); } while(0)   /*!< Configures SCL pin as output */
/*lint -restore */

#define TWI2_SDA_READ() ((NRF_GPIO->IN >> TWI2_MASTER_CONFIG_DATA_PIN_NUMBER) & 0x1UL)                     /*!< Reads current state of SDA */
#define TWI2_SCL_READ() ((NRF_GPIO->IN >> TWI2_MASTER_CONFIG_CLOCK_PIN_NUMBER) & 0x1UL)                    /*!< Reads current state of SCL */

#define TWI2_DELAY() nrf_delay_us(4) /*!< Time to wait when pin states are changed. For fast-mode the delay can be zero and for standard-mode 4 us delay is sufficient. */
